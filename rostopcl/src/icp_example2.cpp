...
void  hsv_objectdetection(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, const sensor_msgs::ImageConstPtr& image1){
	...
	for(int row=0;row<cv_ptr->image.rows;row++){
		for(int column=0;column<cv_ptr->image.cols;column++){
			if(hsv_range_mask.at<uchar>(row+1,column+1)<127){
				cloud->points[row*cv_ptr->image.cols+column].x = std::numeric_limits<float>::quiet_NaN();
				cloud->points[row*cv_ptr->image.cols+column].y = std::numeric_limits<float>::quiet_NaN();
				cloud->points[row*cv_ptr->image.cols+column].z = std::numeric_limits<float>::quiet_NaN();
			}
		}
	}
	std::vector<int> temp; 
...
{	
	...
	if(!model_is_create){
		...
		hsv_objectdetection(cloud_icp_model,image1);
	...
	/*
	//sensor_msgs::PointCloud2 pcl_to_ros_pointcloud1;
	...
	////////Test
	//pcl::copyPointCloud(*cloud_icp_model,*cloud_now);
	////for(int i=0;i<cloud_now->width*cloud_now->height;i++){
	//	cloud_now->points[i].x += 3;
	//	cloud_now->points[i].y += 4;
	//	cloud_now->points[i].z += 5;
	//}
	...
	printf("Filtered point Size: %d %d %d\n",cloud_now->points.size(),cloud_now->width,cloud_now->height);
	*/
	hsv_objectdetection(cloud_now,image1);
...
int   main (int argc, char** argv)
{
	...
	path.header.frame_id="/map";
     ...
     x0=(float)0.0;	yy0=(float)0.0;	z0=(float)0.0;
     ///Synchonizer
